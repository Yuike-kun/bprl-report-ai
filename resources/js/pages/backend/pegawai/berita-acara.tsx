...
export default function BeritaAcara({
    ...
    useEffect(() => {
        ...
        if (konsultasi) {
            setForm((prev) => ({
                ...prev,
                requester_name:
                    prev.requester_name || konsultasi?.nama_pemohon || '',
                requester_position:
                    prev.requester_position ||
                    konsultasi?.jabatan_pemohon ||
                    '',
                legal_entity_name:
    ...
    const setFile = (key: string, val: File[]) =>
    ...
    const docsFor = (slot: string) =>
        existingDocs.filter((d) => d.document_type === slot);
    ...
    const goToStep2 = () => {
        if (
            ...
            alert(
                'Harap lengkapi semua field wajib di Langkah 1 terlebih dahulu.',
            );
            return;
        }
        setStep(2);
    ...
    const handleSubmit = () => {
        ...
        Object.entries(files).forEach(([key, arr]) => {
            if (
                key === 'dokumentasi_konsultasi' ||
                key === 'persyaratan_lainnya'
            ) {
                        ...
                        >
                            ...
                            <div className="min-w-0">
                                ...
                                <p className="text-[10px] text-slate-400">
                                    {n === 2 &&
                                    form.consultation_stage !== 'asistensi'
                                        ? 'Langkah 2 (Khusus Asistensi)'
                                        : `Langkah ${n}`}
                                </p>
                            </div>
                            {idx < 1 && (
                        ...
                        >
                            <div className="grid gap-4 sm:grid-cols-3">
                                ...
                                <div>
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        />
                                        <SelectInput
                                            value={form.planned_area_unit}
                                            onChange={(v) =>
                                                set('planned_area_unit', v)
                                            }
                                            options={[
                                                { value: 'Ha', label: 'Ha' },
                                                { value: 'Km', label: 'Km' },
                            ...
                            <div>
                                ...
                                <p className="mt-1 text-[11px] text-slate-400">
                                    Format: Bujur (lon), Lintang (lat) — satu
                                    titik per baris
                                </p>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                                    {OWNED_DOCUMENTS.map((doc) => {
                                        const checked =
                                            form.owned_documents.includes(doc);
                                        return (
                                            <label
                                                key={doc}
                                ...
                                {form.owned_documents.includes('Yang lain') && (
                                    <div className="mt-2">
                                        <TextInput
                                            value={form.owned_documents_other}
                                            onChange={(v) =>
                                                set('owned_documents_other', v)
                                            }
                        ...
                        <SectionCard title="Lampiran Asistensi" icon={Upload}>
                            <div className="grid gap-5 sm:grid-cols-2">
                                ...
                                <div>
                                    ...
                                    <ExistingFiles
                                        docs={docsFor('data_kondisi_terkini')}
                                        onRemove={removeExistingDoc}
                                    />
                                    <FileUpload
                                        label="Upload File"
                                        ...
                                        onChange={(f) =>
                                            setFile('data_kondisi_terkini', f)
                                        }
                                    />
                                </div>
                                <div>
                                    ...
                                    <ExistingFiles
                                        docs={docsFor('persyaratan_lainnya')}
                                        onRemove={removeExistingDoc}
                                    />
                                    <FileUpload
                                        label="Upload File"
